...
import subprocess
import shutil
from .geo_utils import bbox_to_xyz, tile_edges, get_bounding_box_from_center_coords
...
class MapDownloader():
  ...
  def download_tile(x, y, z, tile_server):
    url = MapDownloader.tile_server.replace(
      "{x}", str(x)).replace(
      "{y}", str(y)).replace(
      "{z}", str(z)
    )
    path = f'{MapDownloader.temp_dir}/{x}_{y}_{z}.png'
  ...
  @staticmethod
  def exec(z: int, center_lat: float, center_lon: float): None
    # zoom = 18
    # center_lat=-7.97699
    ...
    for x in range(x_min, x_max + 1):
      for y in range(y_min, y_max + 1):
        print(f"{x},{y}")
        png_path = MapDownloader.download_tile(x, y, zoom, tile_server)
        MapDownloader.georeference_raster_tile(x, y, zoom, png_path)
        # time.sleep(1)
    ...
    print("Merging tiles")
    output_file=f'/merged-lonmn-{lon_min}_lonmx-{lon_max}_latmn-{lat_min}_latmx-{lat_max}_z-{zoom}_{MapDownloader.tile_host}-{MapDownloader.tile_type}.tif'
    output_filename= MapDownloader.output_dir + output_file
    if os.path.exists(output_filename):
      print('deleting existing file: '+output_filename);
      os.remove(output_filename)
    MapDownloader.merge_tiles(MapDownloader.temp_dir + '/*.tif', output_filename)
    print("Merge complete")
    shutil.rmtree(MapDownloader.temp_dir)
    os.makedirs(MapDownloader.temp_dir)
